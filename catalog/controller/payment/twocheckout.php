...
class ControllerPaymentTwoCheckout extends Controller {
	public  function index() {
		...
		$data['street_address2'] = $order_info['payment_address_2'];
		...
		if ($this->cart->hasShipping()) {
			...
			$data['ship_street_address2'] = $order_info['shipping_address_2'];
			...
			$data['ship_street_address2'] = $order_info['payment_address_2'];
	...
	public function callback() {
		...
		if (strtoupper(md5($this->config->get('twocheckout_secret') . $this->config->get('twocheckout_account') . $order_number . $this->request->request['total'])) == $this->request->request['key']) {
			if ($this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false) == $this->request->post['total']) {
				$this->model_checkout_order->addOrderHistory($this->request->request['cart_order_id'], $this->config->get('twocheckout_order_status_id'));
			} else {
				$this->model_checkout_order->addOrderHistory($this->request->request['cart_order_id'], $this->config->get('config_order_status_id'));// Ugh. Some one've faked the sum. What should we do? Probably drop a mail to the shop owner?
			}
